...
fun StitchScreen() {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Smart Stitch (Android)") }) }) { padding ->
        Column(
            ...
            Text("Pilih folder sumber (INPUT) & tujuan (OUTPUT). App menyalin dari SAF ke cache agar Python bisa memproses.", style = MaterialTheme.typography.bodySmall)

            Row(verticalAlignment = Alignment.CenterVertically) {
                Button(onClick = { pickInput.launch(null) }) { Text(if (inputUri==null) "Pilih Folder INPUT" else "Ganti Folder INPUT") }
                Spacer(Modifier.width(8.dp))
                Text(inputUri?.lastPathSegment ?: "Belum dipilih")
            }
            Row(verticalAlignment = Alignment.CenterVertically) {
                Button(onClick = { pickOutput.launch(null) }) { Text(if (outputUri==null) "Pilih Folder OUTPUT" else "Ganti Folder OUTPUT") }
                Spacer(Modifier.width(8.dp))
                Text(outputUri?.lastPathSegment ?: "Belum dipilih")
            }

            OutlinedTextField(splitHeight, { splitHeight = it }, label = { Text("Split Height") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("Output Type")
                Spacer(Modifier.width(8.dp))
            ...
            OutlinedTextField(unitImages, { unitImages = it }, label = { Text("Unit Images (Low RAM)") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("Width Enforce: ")
                Spacer(Modifier.width(8.dp))
            ...
            if (widthEnforce==2) {
                OutlinedTextField(customWidth, { customWidth = it }, label = { Text("Custom Width") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
            }
            OutlinedTextField(sensitivity, { sensitivity = it }, label = { Text("Sensitivity [0-100]") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
            OutlinedTextField(ignorable, { ignorable = it }, label = { Text("Ignorable Border Pixels") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
            OutlinedTextField(scanStep, { scanStep = it }, label = { Text("Scan Line Step [1-20]") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
                    ...
                    scope.launch {
                        try {
                            val cacheIn = withContext(Dispatchers.IO) {
                                val dir = java.io.File(context.cacheDir, "input"); dir.deleteRecursively(); dir.mkdirs()
                                copyFromTree(context, inUri, dir); dir.absolutePath
                            }
                            val cacheOut = withContext(Dispatchers.IO) {
                                val dir = java.io.File(context.cacheDir, "output"); dir.deleteRecursively(); dir.mkdirs()
                                dir.absolutePath
                            }
            ...
            OutlinedTextField(value = logText, onValueChange = {}, label = { Text("Log") }, readOnly = true, modifier = Modifier.fillMaxWidth().height(200.dp))
        }
    }
}
